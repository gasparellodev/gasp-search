...
- **`<AboutHeroEditorial manifestAboutUrl>`** — `/sobre/page.tsx` resolve.
...
| `BanksStrip.tsx` | **Server Component (#219; consolidado em #299).** Strip compartilhada de 7 bancos parceiros com PNGs locais em `public/assets/banks/*`. Ícones têm dimensões fixas 40×40 para evitar CLS e `alt` como fallback de asset. **#299:** `<BanksStrip>` agora é renderizado APENAS pelo `<HomeBanksPartners>` (1 lugar canônico). Footer global e `<HomeFinancingWidget>` deixaram de instanciar a strip — o widget mantém uma copy curta + link âncora `#bancos-parceiros`. |
| ~~`PaymentStrip.tsx`~~ | **REMOVIDO em #295 (Footer + ContactSection).** Cliente decidiu remover a seção de métodos de pagamento. Assets `public/assets/payment/*.svg` mantidos no repo pra rollback rápido — basta recriar o componente apontando pra eles. |
...
| `about/` | Sections da rota `/sobre` (`AboutHeroEditorial`, `AboutMissionVision`, `AboutWarrantyDeepdive` — issue #229). Server-only. Ver `about/CLAUDE.md`. |
| `contact/` | Sections da rota `/contato` (`ContactSection`, `ContactDualPane`, `BusinessHours`, `WhatsAppDirectCard` — issue #230). Server-only com `<SiteForm variant="contact">` aninhado. Ver `contact/CLAUDE.md`. |
| `advertise/` | Section da rota `/anunciar` (`AdvertiseSection` server + `AnnounceHero`/`AnnounceProcessExplanation` server + `AnnounceForm` client — issues #163/#231). Stepper de anúncio com schema Zod compartilhado, `car_target_slug`, compressão client-side e upload assinado para `tradein-photos`. Ver `advertise/CLAUDE.md`. |
