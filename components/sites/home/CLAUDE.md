...
| `HomeFinancingWidget.tsx` | **Client (Sprint 4 / #H2 — issue #222; consolidação #299).** Diferencial competitivo nº 2 — calculadora INLINE. Split 6/6 desktop: copy à esquerda; form + output à direita. **#299:** `<BanksStrip>` foi REMOVIDO do lado esquerdo (consolidado em `<HomeBanksPartners>`); o slot virou uma copy curta `"Simulamos em 7 bancos parceiros. Veja a lista completa →"` com link âncora `#bancos-parceiros`. Inputs: price text mask vanilla (`useState` + `formatBRL`, sem `react-input-mask`/`imask`), slider entrada 0-50% step 5%, select prazo `{12, 24, 36, 48, 60}`. Output em `aria-live="polite"` com `min-height` fixo (layout shift 0). Edge cases: price=0 → "—"; downPaymentPct=100 → "Sem financiamento". `useDeferredValue` em vez de `setTimeout` (React 19 idiom). DISCLAIMER_TEXT obrigatório (CDC + Bacen) abaixo do output. CTA → `buildWhatsAppLink({template: 'financing', ...})` com `finance` context. |
| `HomeTradeinWidget.tsx` | **Server-only (Sprint 4 / #H2 — issue #222; separação Trade-in/About — issue #298).** Split 6/6 desktop: foto editorial + h2 + body + 2 CTAs. **Foto chain (#298):** `manifestTradeinUrl ?? tradeinImageUrl ?? '/assets/about/porsche-model.png'` — slot dedicado em `VisualIdentityManifest.tradein_url` + brand asset opcional `brand_assets.tradein_image_url` (campos novos, optional+nullable). Desacopla a Home do `about_url`/`about_image_url`, que continuam alimentando `<HomeWarrantySection>` exclusivamente — resolve a duplicação visual reportada pelo cliente Ducarmo (mesma foto editorial em "Seu carro" e "Por que comprar"). Sites legados sem o campo populado caem no fallback local, distinto da imagem de Warranty. CTAs: "Avaliar meu carro" → `/sites/<slug>/anunciar` (primary preto) + "WhatsApp" → `buildWhatsAppLink({template: 'tradein', ...})` (outlined verde). |
...
| `HomeBanksPartners.tsx` | **Server-only (Sprint 4 / #H3 — issue #223; canônico após #299).** Wrapper sobre `<BanksStrip>` shared com h2 `"Bancos parceiros para financiar seu próximo carro"` + caption. **#299 (WP-B):** ÚNICA fonte de verdade visual dos 7 logos — `<BanksStrip>` foi removido do footer e do `<HomeFinancingWidget>` pra eliminar a duplicação tripla reportada pelo cliente. `<section id="bancos-parceiros">` ancora link interno do `<HomeFinancingWidget>` ("Veja a lista completa →"); `scroll-mt-24` reserva o offset do header sticky no scroll suave. |
...
| `HomeFAQSection.tsx` | **Client (Sprint 4 / #H3 — issue #223; refactor #232).** Wrapper fino sobre `<SiteFAQ>` compartilhado, usando as 8 perguntas hardcoded em `lib/sites/faq-template.ts` (range PO refinement: 7-10). **IMPORTANTE — SEM JSON-LD `FAQPage` Schema:** DESIGN.md anti-pattern explícito; Google penaliza FAQPage em business sites desde 2023. Test (`HomeFAQSection.test.tsx`) checa que `<script type="application/ld+json">` NÃO contém "FAQPage". |
...
- `../SiteFAQ` — accordion FAQ compartilhado extraído em #232.
