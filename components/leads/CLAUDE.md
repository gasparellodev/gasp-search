...
5. **`LeadDetailDrawer` é wrapper Sheet sobre `<LeadTabs mode="inline">`**
   ...
   apenas com header (`SheetTitle` + descrição), footer (botão Fechar) e
   delega tabs/edição para `<LeadTabs />`.
...
| `lead-detail-drawer.tsx` | Client | Wrapper Sheet sobre `<LeadTabs mode="inline">` (refatorado #136). Mantém só o header (SheetTitle + descrição), footer (Fechar) e passa `<LeadSiteCardClient />` como slot. |
