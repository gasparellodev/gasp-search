...
| `lead-site-card-actions.tsx` | Client | Cluster de botões interativo do `<LeadSiteCard />`. Três `useTransition` independentes (generate / archive / restore) pra evitar que um spinner bloqueie outro botão. **Estado `none`/`draft`** → "Gerar site agora". **Estado `archived`** → "Restaurar" (chama `restoreLeadSite`, #169). **Estados `published`/`sent`** → cluster com Pré-visualizar (link target=_blank), Copiar (clipboard), Editar (abre `<LeadSiteEditModal>`, #168), Regerar (chama `generateLeadSite`, #159), Arquivar (abre `<AlertDialog>` de confirmação destrutiva → confirma chama `archiveLeadSite`, #169) e Enviar via WhatsApp (disabled até #171). Toda async dispara toast `sonner` em sucesso/erro com mensagens PT-BR. Confirm dialog usa Radix `AlertDialog` (role=alertdialog, focus trap, ESC cancela) — coberto por jest-axe nos testes. |
