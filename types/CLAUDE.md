...
| Path | Propósito |
|---|---|
| `database.ts` | Schema Supabase hand-written enquanto não há `gen types` remoto aplicado. Inclui `lead_sites` (signed_at #199, visual_identity #215), `lead_form_submissions` (#223) e `consent_logs` (#234, auditoria do banner LGPD com action union + categories JSON). Enums (`search_source`, `search_status`, `lead_stage`, `whatsapp_status`, `campaign_*`, `lead_message_*`, `lead_site_status`) e helpers `Tables<T>`/`TablesInsert<T>`/`TablesUpdate<T>`/`Enums<T>`. Novas migrations precisam de edit manual aqui no mesmo PR; `tests/unit/types/database.test.ts` cobre o shape. |
