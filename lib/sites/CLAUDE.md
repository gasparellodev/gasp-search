...
3. **Server-only por padrão.** Helpers que tocam Supabase/Anthropic
   ...
   **Exceções documentadas:** `static-map.ts`, `site-assets.ts` e
   `motion.ts` são intencionalmente client-runnable — não importam
   `server-only` nem declaram `'use client'` no topo (o caller é quem
   declara `'use client'`). Adicionar nova exceção exige justificativa
   no row da tabela abaixo.
...
| Path | Propósito |
|---|---|
| `slug.ts` | `generateUniqueSlug(business_name, client)` — `<nanoid8>-<base>` com retry × 5 contra `lead_sites.slug`. Lança `SlugCollisionError` em exhaustão. Alfabeto sem `0/o/1/i/l`. **#232:** exporta também `slugifyVehicle({ brand, model, year, id })` → `{brandSlug}-{modelSlug}-{year}-{idShort4}` para novos `cars[].slug`; chars PT-BR viram ASCII. Colisão do `idShort4` é aceita no MVP e documentada nos testes. **Sprint B1 (onsite flow):** exporta `validateCustomSlug(input)` (pure — regex `^[a-z0-9](?:[a-z0-9-]{1,38}[a-z0-9])$`, sem `--`, blacklist de rotas internas `admin/api/leads/...`, retorna `CustomSlugValidationResult { ok, code, message, normalized }`), `suggestSlugFromName(business_name)` (wrapper de `slugify` truncado em 40 chars) e `isCustomSlugAvailable(candidate, client)` (count+head SELECT em `lead_sites.slug`). Consumido pelo `<LeadSitePreGenModal>` (validação inline em tempo real) e pelo `generateLeadSite(leadId, { customSlug })` (validação + reserva antes de chamar Anthropic). |
...
| `generate-faq.ts` | **Server-only.** Phase 7 / Frente 04 GEO/AI — issue #G3. `generateFaqContent(input: GenerateFaqInput): Promise<FaqEntry[]>` — 1 call Anthropic Sonnet 4.6 com tool use forçado `emit_faqs` + system prompt cacheado. Output Zod-validado (exatamente 8 itens, question 5-200 chars, answer 20-800 chars). Lança `FaqGenerationError { code, retryable, cause }` nos caminhos de falha (mesmo contrato de `GenerationError` em `generate-copy.ts`). Exporta `FAQS_TARGET_COUNT=8`, `FaqGenerationError`, `SYSTEM_PROMPT_FAQ`, `GenerateFaqInput`. **OUT OF SCOPE (follow-ups):** (1) Persistência — `SiteVariablesV2` não tem campo `faq`; criar coluna `lead_sites.faq_generated JSONB NULL` via migration. (2) Auto-trigger em `signLeadSite` — requer persistência primeiro. (3) Botão na admin UI — follow-up wave. Caller canônico: `app/actions/generate-faq.ts` (`generateFAQ(siteId)`). |
...
| `url-safety.ts` | **Pure / Edge-compatible (#244).** `isPrivateOrLinkLocalHost(input)` bloqueia fetch server-side/Edge para hosts privados ou metadata: RFC1918 (`10/8`, `172.16/12`, `192.168/16`), link-local `169.254/16`, loopback `127/8`/`::1`/`localhost`, `0.0.0.0` e hostnames `metadata` / `metadata.google.internal`. Também detecta hostname com IPv4 privado/link-local embutido no sufixo (`attacker.com.169.254.169.254`). Não faz DNS lookup para manter compatibilidade com Edge runtime. |
...
| `static-map.ts` | **Pure (sem `server-only`).** Phase 7 / Wave 5 O2 — issue #230. `buildStaticMapUrl({ apiKey, placeId, lat, lng, address })` monta URL Google Static Maps sem fazer fetch, priorizando `placeId` → `lat,lng` → endereço textual; retorna `null` sem API key ou sem alvo. `buildGoogleMapsPlaceHref(address)` monta fallback externo `google.com/maps/place/?q=...`. |
...
| `motion.ts` | **Client-runnable (sem `server-only`, sem `'use client'` no topo — caller declara).** Phase 7 / WP1 — issue #290. Foundation para o sprint de refinamento premium: `prefersReducedMotion(): boolean` SSR-safe (sem memoização — preferência muda em runtime), `loadGsap(): Promise<{gsap, ScrollTrigger}>` com dynamic import + `registerPlugin(ScrollTrigger)` chamado uma única vez via memoização module-level (`p1 === p2` em chamadas subsequentes) + rejeição com mensagem clara em SSR, `loadAnime(): Promise<typeof animejs>` com mesmo padrão de dynamic import + memoização + SSR rejection. **anime.js v4 — usar named exports** (`mod.animate({...})`, `mod.stagger(120)`); a API legada `anime(...)`/`anime.stagger(...)` NÃO existe nessa versão. Consumido por WPs futuros (#291 AnnouncementBar, #292 Hero, #296 reveals). Bundle: GSAP + ScrollTrigger + anime.js só entram em chunks dinâmicos quando o caller importa o helper — `/sites/[slug]` SSR-only não infla First Load JS. |
| `announcement.schema.ts` | `AnnouncementSchema` Zod do `AnnounceForm` (`/anunciar` em #163/#231). Compartilhado entre Client Component e Server Action `submitAnnouncement` — fonte única de verdade. Campos: `marca`, `modelo`, `ano` (1980..currentYear+1), `km` (≥0), `preco` (≥0, opcional/null), `nome`, `telefone` (10–13 dígitos pós-strip), `email`, `mensagem` (opcional, max 1000), `car_target_slug` opcional e `lgpd_consent: literal(true)`. |
| `announcement-security.ts` | **Server-only (#231).** Assina/verifica contexto do form de anúncio e token de upload via HMAC SHA-256 quando `SITE_FORM_HMAC_SECRET` está preenchido; sem secret, `verify*` faz fail-open para preservar previews/CI sem segredo. Também valida same-origin contra `NEXT_PUBLIC_APP_URL`. Não importar em Client Components. |
| `tradein-upload.ts` | **Server-only (#231).** Validação pura de upload para fotos do trade-in: extensões/MIME permitidos (`jpg/jpeg/png/webp/heic`), tamanho máximo 5MB e magic bytes enviados pelo client. `buildTradeinPhotoPath({leadId,index,timestamp,ext})` gera path no bucket privado `tradein-photos` como `<leadId>/<index>-<timestamp>.<ext>`. |
...
| `stock-search-params.ts` | **Pure (sem `server-only`).** Phase 7 Sprint 4 / #H1 (issue #221) + Sprint 5 / #E1 (issue #224). Fonte única de verdade pro querystring do quick search da Home + filtros do `/estoque`. Mantém `serializeQuickSearch({brand, model, priceMax})` (`m`, `model`, `p`) e `parseQuickSearch(input)`. #224 adiciona `StockFilters`, `parseStockFilters`, `serializeStockFilters`, `applyStockFilters`, `buildStockFilterFacets`, `countActiveStockFilters`. Short keys canônicas: `q`, `m`, `model`, `c`, `pmin/pmax`, `imin/imax`, `ymin/ymax`, `kmmin/kmmax`, `tr`, `fl`, `cor`; `parseStockFilters` preserva passthrough desconhecido (`page`, `sort`) e aceita legado `p`/`categoria`; `serializeStockFilters` emite o contrato novo (`pmax`, `c=pickup`). `applyStockFilters` é client-safe, filtra em memória por brand/model/category heurística, ranges, câmbio, combustível e cor; parcela usa `calculateInstallment` com defaults de card. |
...
| `warranty-process.ts` | **Pure (sem `server-only`).** Phase 7 / Wave 5 O1 — issue #229. Exporta `WARRANTY_PROCESS` com 3 cards canônicos do deep-dive de garantia da página Sobre (`Vistoria 100 pontos`, `Garantia mecânica de 3 meses`, `Suporte pós-venda direto`). Usa nomes de ícones Lucide como dados (`Search`, `ShieldCheck`, `Headphones`) e é consumido por `<AboutWarrantyDeepdive>`. Não reutilizar `WARRANTY_BULLETS` aqui; os contextos de copy são diferentes. |
...
| `find-similar-cars.ts` | **Pure (sem `server-only`).** Phase 7 / Sprint 6 / #D3 — issue #228. Exporta `findSimilarCars(cars, current, limit=4): { similar, fallback }` — algoritmo cross-conversion do bloco "Veículos similares" no detalhe. Regras AC: (1) Excluir `current` por slug, (2) Primary match same `category` com fallback `brand` quando `current.category===undefined` OU nenhum carro casa categoria, (3) brand match **case-insensitive**, (4) Filtro faixa de preço ±20% só quando `current.price>0` (carros com `price===null` no pool passam o filtro), (5) Faixa que zera o primary relaxa pra preservar primary inteiro (anti-outlier), (6) Sort por proximidade asc; carros com `price===null` pesam `Infinity` (vão pro fim); sem sort quando `current.price===null` (sem âncora), (7) `fallback` completa até `limit` com top-priced (preço asc DESC), excluindo similar + filtrando `price>0`. Retorna duas listas distintas pra UI distinguir badge "Você também pode gostar" — `<DetailSimilarVehicles>` decide rendering. **Coverage 16 casos** em `tests/unit/lib/sites/find-similar-cars.test.ts`. |
| `detail-faq-templates.ts` | **Pure (sem `server-only`).** Phase 7 / Sprint 6 / #D3 — issue #228. Exporta `DETAIL_FAQ_TEMPLATE: readonly DetailFaqEntry[]` (5 perguntas — range PO 4-6, sweet spot 5) + `buildDetailFaqItems(car: { brand, model, year }): DetailFaqEntry[]` que interpola placeholders `{brand}/{model}/{year}` (substituição global via `split().join()` — múltiplas ocorrências). Preserva case original do car (não down-case). Consumido por `<DetailFaqVehicle>` via `<SiteFAQ>` shared. **Não emite JSON-LD `FAQPage`** (anti-pattern Google p/ business sites). Mudanças de copy requerem PO sign-off. Snapshot-locked em `tests/unit/lib/sites/detail-faq-templates.test.ts` pra defender drift. |
