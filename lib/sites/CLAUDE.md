...
| `dispatch-site-preview.ts` | **Server-only.** `dispatchSitePreview({ supabase, service, userId, leadId, sendImpl? })` — núcleo extraído de `sendLeadSiteWhatsApp` (#171) pra ser reusado pelo processor de campanhas `type='site_preview'` (#172). Pipeline: fetch `lead_sites` por `lead_id` → status guard (`'published'`/`'sent'`) → fetch `lead.name` → render `SITE_PREVIEW_TEMPLATE` → `sendWhatsAppMessage` → update `lead_sites.status='sent'` via service-role. Retorno discriminated union (`'no_site' \| 'invalid_status' \| 'render_error' \| 'whatsapp_error' \| 'db_error'`). **Não faz auth nem cache invalidation** — caller é responsável (Server Action `sendLeadSiteWhatsApp` em `app/actions/lead-site.ts` continua dono dessas duas responsabilidades). |
