...
| Path | Propósito |
|---|---|
| `generate-faq.ts` | `generateFAQ(siteId: string): Promise<GenerateFAQResult>` — Server Action Phase 7 / Frente 04 GEO/AI (#G3). Auth via `createServerSupabase()` → fetch `lead_sites` por ID (RLS enforça ownership) → `readSiteVariablesSafe` → extrai `business_name`, `cars[].brand` (deduplicated) e `address.{city,state}` → delega para `lib/sites/generate-faq.ts:generateFaqContent`. Retorno discriminated union `GenerateFAQResult = { ok: true; faqs: FaqEntry[] } \| { ok: false; code: string; message: string }`. **Sem persistência V1** — follow-up necessário para `lead_sites.faq_generated JSONB NULL`. Auto-trigger em `signLeadSite` e botão admin UI também são follow-ups. |
...
| `site-announcement.ts` | `submitAnnouncement(siteId, payload, extras?)` + `requestUploadUrl(siteId, input)` — Server Actions do `<AnnounceForm>` público (#163/#231). `submitAnnouncement` valida via `AnnouncementSchema`, honeypot `_hp_company`, same-origin e HMAC opcional (`SITE_FORM_HMAC_SECRET`), aplica rate limit Map 3/h/IP, persiste em `lead_form_submissions`, audita LGPD em `consent_logs` (`version='tradein_submission_v1'`) e retorna `{ ok: true; leadId; uploadToken } \| { ok:false; error }`. `requestUploadUrl` valida token, lookup da submission, extensão/MIME/tamanho/magic bytes e assina upload no bucket privado `tradein-photos`. Logs PII-safe; nunca expor erro cru do banco ao cliente. |
...
| `lead-site.ts` (discardLeadSiteDraft) | **Sprint A3 onsite flow.** `discardLeadSiteDraft(leadSiteId)` — recovery para drafts presos com `generation_error`. Pipeline: auth → fetch (RLS) → status guard (`status='draft'` AND `generation_error != null`) → hard-delete via service_role → `updateTag('site:<slug>')` + `revalidatePath`. Retorno `DiscardLeadSiteDraftResult = { ok: true } \| { ok: false; error: 'auth' \| 'not_found' \| 'invalid_status' \| 'db_error'; message }`. UI mostra o botão "Descartar rascunho" apenas no estado `draft+error` (ver `<LeadSiteCardActions>` cluster `lead-site-draft-error-cluster`). Hard-delete (vs soft archive) libera o slug pra próxima tentativa do operador sem herdar brand assets/copy cached. |
...
| `status !== 'draft'` OU `generation_error === null` | `invalid_status` |
| Delete DB error | `db_error` |

Sucesso → `{ ok: true }`. **Hard-delete** da row de `lead_sites` (vs soft archive em `archiveLeadSite`). Slug liberado pra próxima `generateLeadSite`.
