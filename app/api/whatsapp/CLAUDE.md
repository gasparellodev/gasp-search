...
9. **Presença é volátil (#123).** `presence.update` do webhook resolve o lead por phone/whatsapp dentro do tenant e grava Redis TTL 60s via `lib/whatsapp/presence.ts`. `/presence/[leadId]` sempre valida ownership do lead antes de ler Redis. `/typing` aceita só `typing|paused` e também valida ownership antes de registrar o estado local usado pela UI.
...
| `presence/[leadId]/route.ts` | GET retorna `{presence,lastSeen}` do Redis para o lead autenticado. Valida `leads.id + user_id` antes de ler a chave. |
| `typing/route.ts` | POST registra `typing`/`paused` para o lead autenticado. Body `{leadId: uuid, presence}`; usado pelo composer com debounce 2s. |
| `webhook/route.ts` | POST público (HMAC opcional + fallback de auth via lookup `evo_instance_v2` → `evo_instance` legacy em `whatsapp_instances`). Processa `messages.upsert` (inbound, com transição automática `new`/`contacted` → `in_conversation`), `message.status` (atualiza `lead_messages.status` por `(whatsapp_msg_id, user_id)`), `connection.update` (atualiza `whatsapp_instances` por `user_id`) e `presence.update` (resolve lead e grava Redis TTL 60s). Usa `service_role` — **#130:** lookup do `userId` roda ANTES de qualquer processamento (inclusive eventos `unknown`) e cada update persiste `.eq('user_id', ctx.userId)` (defense-in-depth anti-IDOR). |
