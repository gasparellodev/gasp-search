...
/**
 * Error boundary da rota `/sites/[slug]/sobre` (issue #202).
 * Ver `app/sites/[slug]/error.tsx` para política completa.
 */
export default function SobreError({ error, reset }: ErrorBoundaryProps) {
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      console.error("[site:sobre:boundary]", error);
    }
  }, [error]);
