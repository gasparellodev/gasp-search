/**
 * Fixture v1 — shape **flat antigo** de `SiteVariables` (pré-issue #197).
 *
 * Usado em `tests/unit/lib/sites/migrate-variables.test.ts` para validar
 * que o helper `readSiteVariables` aceita registros v1 e devolve v2 via
 * fallback gracioso.
 *
 * Campos refletem `SiteVariablesV1` em `types/lead-site.ts`. **NÃO** tem
 * `schema_version` — discriminador do helper `isV1` é justamente "ausência
 * dessa key".
 *
 * **Tipo: cast para `Record<string, unknown>` propositalmente** — o V1
 * histórico não tinha `category` nem `plates_visible` em `cars[]`, mas
 * `SiteCar` v2 (output type) exige ambos. Tipá-los aqui mascararia o
 * caminho real de migration. O fixture é input para `migrateV1ToV2`,
 * que adiciona os campos com defaults.
 */

export const fixtureSiteVariablesV1: Record<string, unknown> = {
  // Globais
  ...
  facebook_url: null,
  youtube_url: null,
  address_line: "Av. Paulista, 1000 — Bela Vista, São Paulo - SP, 01310-100",
  ...
  // Contato
  contact_hero_image_url: "/assets/contact/bmw-m2.png",
